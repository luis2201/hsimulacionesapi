...
console.log('Datos de la reserva recibidos:', datos);
  ...
  setEstadoManual: (req, res) => {
    const { id } = req.params;
    let { EstadoManual } = req.body;

    const estadosPermitidos = ['SUSPENDIDA', 'REPROGRAMADA', null];

    // Si se envía una cadena vacía o null, establecer como null
    if (EstadoManual === '' || EstadoManual === null) {
      EstadoManual = null;
    }

    if (!estadosPermitidos.includes(EstadoManual)) {
      return res.status(400).json({ error: 'EstadoManual inválido' });
    }

    Reserva.updateEstadoManual(id, EstadoManual, (err) => {
